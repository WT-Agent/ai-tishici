  ...
  <div class="app-container">
    ...
    <main ref="inputCardRef" class="glass-card input-group">
      ...
      <div class="options-row" style="display: grid; grid-template-columns: repeat(auto-fit, minmax(200px, 1fr)); gap: 1rem;">
        <div class="selector-group">
          <label class="selector-label">目标模型</label>
          <div class="style-selector">
            <button 
        ...
        <div class="selector-group">
          <label class="selector-label">优化框架</label>
          <div class="style-selector">
            <button 
      ...
      <div class="selector-group">
        <div style="display: flex; justify-content: space-between; align-items: center;">
          ...
          <div style="display: flex; gap: 0.5rem;">
            ...
            <button class="text-link-btn" @click="showGuideModal = true">Prompt 提示词工程与大模型指令指南</button>
          </div>
        </div>
        <textarea 
          v-model="userInput" 
...
const appTitle = ref(appConfig.title || '网腾无限AI - Prompt 提示词工程与指令专家');
...
const presetOptions = [
  "将粗糙问题重构为企业级 System Role 专家 Prompt",
  "Midjourney v6 极致写实摄影与光影参数提示词",
  "约束 ChatGPT 严格输出 JSON/Markdown 格式 Prompt",
  "让 AI 进行复杂逻辑推理的思维链 (CoT) 提示词"
];
...
const metricsList = [
  {
    "key": "promptStructureRigor",
    "label": "Prompt结构严谨度"
  },
  {
    "key": "systemInstructionPrecision",
    "label": "系统指令精准度"
  },
  {
    "key": "outputFormatConstraint",
    "label": "输出格式约束力"
  },
  {
    "key": "aiModelCompatibility",
    "label": "模型自适应兼容性"
  },
  {
    "key": "efficiencyBoostRatio",
    "label": "效率提升转化率"
  }
];
const guideRules = [
  {
    "title": "明确 Role 角色与 Context",
    "advice": "给 AI 设定具体身份（如“你是一位 10 年经验资深架构师”）与清晰场景背景，限制输出范围",
    "avoid": "切忌给出“请帮我写一篇文章”这种缺乏上下文的抽象指令"
  },
  {
    "title": "用 Positive 替代 Negative",
    "advice": "多告诉 AI“应该做什么”、“以 JSON 格式输出”，少用抽象的“不要做某事”",
    "avoid": "避免在指令中充斥大量否定词导致 AI 产生逻辑困惑"
  },
  {
    "title": "提供 Few-Shot 优质示例",
    "advice": "在 Prompt 中附带 1-2 个标准的“输入-输出”示例，模型输出的格式一致性将提升 90%",
    "avoid": "严禁要求复杂的特定格式却不给具体示例"
  }
];
...
const displayResultText = computed(() => {
  ...
  return result.value.replace(/\[TISHICI_SCORES\][\s\S]*?\[\/TISHICI_SCORES\]/g, '').trim();
});

const parseAiScores = (rawText: string) => {
  const match = rawText.match(/\[TISHICI_SCORES\](.*?)\[\/TISHICI_SCORES\]/);
...
const loadHistory = () => {
  try {
    const raw = localStorage.getItem('tishici_history_records');
...
const saveHistory = () => {
  localStorage.setItem('tishici_history_records', JSON.stringify(historyList.value));
};
